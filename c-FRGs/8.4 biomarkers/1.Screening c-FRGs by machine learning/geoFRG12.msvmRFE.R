...
# Makes a plot of average generalization error vs. number of top features
    AddLine <- function(x, col='dodgerblue') {
        ...
        text(which.min(x), min(x, na.rm=T), paste0("n=", which.min(x), " (", 
                                                  format(min(x, na.rm=T), dig=3),")"), pos=2, col='red', cex=1)
    }
...
Plotaccuracy <- function(errors, errors2=NULL, no.info=0.5, 
  ...
  AddLine <- function(x, col='dodgerblue') {
    ...
    text(which.max(x), max(x, na.rm=T), paste0("n=", which.max(x), " (", 
                                              format(max(x, na.rm=T), dig=3),")"), pos=2, col='red', cex=1)
  }
